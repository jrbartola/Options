...
from option_chain import get_option_chain, get_price_quote
...
def analyze_verticals(symbol, contract_type, credit_or_debit, max_strike_width=50):
    ...
    last_price = get_price_quote(symbol)

    if credit_or_debit == 'CREDIT':
        return analyze_credit_spreads(dte_map, contract_type, last_price, max_strike_width)

def analyze_credit_spreads(dte_map, contract_type, last_price, max_strike_width=None):
    result_dte_map = {}
    ...
    for dte, strike_map in dte_map.items():
        ...
        for i in range(len(sorted_strikes) - 1):
            for j in range(i + 1, len(sorted_strikes)):
                ...
                high_option_model = OptionContract(price=last_price,
                ...
                low_option_model = OptionContract(price=last_price,
