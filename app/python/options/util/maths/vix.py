...
def T(dte):
    current_day_timedelta = (
        datetime.combine(date.today() + timedelta(days=1), time()) - datetime.now()
    )
...
def smallest_difference(strikemap):
    ...
    for strike in strikemap:
        ...
        if abs_diff < smallest_diff:
            ...
            diff_info = {
                "strike": strike,
                "call_price": call_midpoint,
                "put_price": put_midpoint,
            }
...
def get_otm_options(k_0, contract_type, strikemap):
    ...
    strike_filter = (
        (lambda s: s > k_0) if contract_type == CALL else (lambda s: s < k_0)
    )
    contract_filter = lambda s: contract_type in strikemap[s]
    sorted_strikes = list(
        filter(
            lambda s: strike_filter(s) and contract_filter(s), sorted(strikemap.keys())
        )
    )
...
def merge_k0(k0, strikemap):
    ...
    return {
        "bid": (call_k0["bid"] + put_k0["bid"]) / 2,
        "ask": (call_k0["ask"] + put_k0["ask"]) / 2,
        "strikePrice": k0,
    }
...
def sigma_squared(t, r, f, k_0, K_i):
    ...
    delta_k_i.extend(
        [
            (K_i[i + 1]["strikePrice"] - K_i[i - 1]["strikePrice"]) / 2
            for i in range(1, len(K_i) - 1)
        ]
    )
    delta_k_i.extend([K_i[-1]["strikePrice"] - K_i[-2]["strikePrice"]])

    k_i_sum = sum(
        (delta_k_i[i] / K_i[i]["strikePrice"] ** 2)
        * np.exp(r * t)
        * ((K_i[i]["bid"] + K_i[i]["ask"]) / 2)
...
def get_t_sig_terms(dte, strikemap):
    ...
    r = 0.045  # TODO: Find a way to get this dynamically

    # Step 1: Find F
    smallest_diff_info = smallest_difference(strikemap)
    f = F(
        smallest_diff_info["strike"],
        r,
        t,
        smallest_diff_info["call_price"],
        smallest_diff_info["put_price"],
    )
    ...
    # Step 3: Find K_i's
    K_i = (
        get_otm_options(k_0, PUT, strikemap)
        + [merge_k0(k_0, strikemap)]
        + get_otm_options(k_0, CALL, strikemap)
    )
...
def vix(symbol):
    data = get_option_chain(symbol, low_dte=23, high_dte=80, min_volume=0)
    dte_map = combine_contract_data(data["dte_maps"][CALL], data["dte_maps"][PUT])
    ...
    weighted_avg = (
        ...
        * sigma_sq1
        * (
            (t2 * MINUTES_PER_YEAR - MINUTES_PER_MONTH)
            / (t2 * MINUTES_PER_YEAR - t1 * MINUTES_PER_YEAR)
        )
        + t2
        * sigma_sq2
        * (
            (MINUTES_PER_MONTH - t1 * MINUTES_PER_YEAR)
            / (t2 * MINUTES_PER_YEAR - t1 * MINUTES_PER_YEAR)
        )
    ) * (MINUTES_PER_YEAR / MINUTES_PER_MONTH)
