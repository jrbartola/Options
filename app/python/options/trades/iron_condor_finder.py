...
from options.trades.filters.common_filters import (
    prob_profit_gt,
    expected_profit_gt,
    credit_percentage_gt,
)
...
def find_iron_condors(symbol, spread_filters=[], **kwargs):
    ...
    for i in range(len(call_spreads)):
        for j in range(len(put_spreads)):
            ...
            iron_condors.append(
                IronCondor(symbol, call_spread, put_spread, dte=call_spread.dte)
            )

    filtered_spreads = reduce(
        lambda spreads, spread_filter: [
            spread for spread in spreads if spread_filter.filter(spread)
        ],
        spread_filters,
        iron_condors,
    )
    spread_data = [
        [
            spread.expected_profit(),
            spread.credit_percentage,
            spread.prob_profit(),
            spread.max_profit,
            spread.max_loss,
        ]
    ...
    df = pd.DataFrame(
        data=spread_data,
        index=[repr(s) for s in filtered_spreads],
        columns=[
            "expectedProfit",
            "creditPercent",
            "probProfit",
    ...
    return df.sort_values(by="expectedProfit", ascending=False)
